...
kafka-topics --create --bootstrap-server 192.168.43.58:9093 replication-factor 1 --partitions 1 --topic SERVER_RECEIVE  --command-config /etc/kafka/secrets/run.properties
kafka-topics --create --bootstrap-server 192.168.43.58:9093 replication-factor 1 --partitions 1 --topic SERVER_SEND  --command-config /etc/kafka/secrets/run.properties
